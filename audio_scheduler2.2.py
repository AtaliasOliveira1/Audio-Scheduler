...
class AudioSchedulerApp:
    def __init__(self, root):
        ...
        self.btn_iniciar = tk.Button(self.frame, text="Iniciar Agora", command=self.iniciar_sequencia)
        self.btn_iniciar.grid(row=2, column=0, sticky="ew", pady=5)

        self.btn_parar = tk.Button(self.frame, text="Parar Agora", command=self.parar_sequencia)
        ...
        self.alert_checkbox.grid(row=6, column=0, columnspan=2, pady=2)

        # --- Área de Agendamento ---
        schedule_frame = tk.LabelFrame(self.frame, text="Agendamento Automático (HH:MM)", padx=5, pady=5)
        schedule_frame.grid(row=7, column=0, columnspan=2, sticky="ew", pady=10)

        tk.Label(schedule_frame, text="Início:").grid(row=0, column=0)
        self.entry_start = tk.Entry(schedule_frame, textvariable=self.start_time_var, width=10, justify="center")
        self.entry_start.grid(row=0, column=1, padx=5)

        tk.Label(schedule_frame, text="Parada:").grid(row=0, column=2)
        self.entry_stop = tk.Entry(schedule_frame, textvariable=self.stop_time_var, width=10, justify="center")
        ...
        self.schedule_checkbox.grid(row=1, column=0, columnspan=4, pady=5)

        # --- Configurações Finais ---
        self.frame.grid_rowconfigure(0, weight=1)
        self.frame.grid_columnconfigure((0, 1), weight=1)

        self.carregar_dados() # Carrega lista E configurações
    ...
    # --- NOVA ESTRUTURA DE SALVAMENTO ---
    def salvar_dados(self):
        # Agora salvamos um dicionário contendo configs E audios
        data = {
            "config": {
    ...
    def carregar_dados(self):
        if os.path.exists(SAVE_FILE):
            try:
                ...
                # Verifica se é o formato novo (dicionário) ou antigo (lista)
                if isinstance(data, dict):
                    # Carrega configurações
                    config = data.get("config", {})
                    self.start_time_var.set(config.get("start_time", "08:00"))
                    self.stop_time_var.set(config.get("stop_time", "18:00"))
    ...
    def atualizar_status(self):
        ...
        if self.scheduling_enabled.get():
            if current_time_str == self.start_time_var.get():
    ...
    def on_closing(self):
        self.parar_sequencia(silent=True)
        self.salvar_dados() # Salva tudo antes de fechar
        self.root.destroy()
